...
function createVerificationService({ store, clock, newId }) {
  ...
  function latestOfKind(memberId, kind) {
  ...
  function submit({ memberId, kind, docType, imageRef, birthDate, fullName, issuedOn }) {
    ...
    const pending = latestOfKind(memberId, kind)
  ...
  function isSingleCertificateValid(document, now = clock()) {
    ...
    return daysBetween(parseDateOnly(document.issuedOn, 'issuedOn'), now) <= SINGLE_CERTIFICATE_VALID_DAYS
  }

  /** 期限切れの独身証明バッジを落とす。参照系から呼ばれる冪等な処理。 */
  function refreshBadges(member, now = clock()) {
    const cert = latestOfKind(member.id, DOCUMENT_KIND.SINGLE_STATUS)
    const valid = isSingleCertificateValid(cert, now)
    if (member.badges.singleCertified !== valid) {
      member.badges.singleCertified = valid
      member.singleCertifiedUntil = valid
        ? addDays(parseDateOnly(cert.issuedOn, 'issuedOn'), SINGLE_CERTIFICATE_VALID_DAYS).toISOString()
        : null
      store.members.save(member)
    }
    return member
  }
  ...
  return {
    /** 本人確認書類の提出。入会に必須。 */
    submitIdentityDocument({ memberId, docType, imageRef, fullName, birthDate }) {
    ...
    submitSingleStatusCertificate({ memberId, imageRef, fullName, issuedOn }) {
      const now = clock()
    ...
    approve({ documentId, reviewerId }) {
      const document = store.documents.find(documentId)
      ...
      const member = getMember(document.memberId)
      const now = clock()
    ...
    reject({ documentId, reviewerId, reason }) {
      const document = store.documents.find(documentId)
      if (!document) throw new NotFoundError('書類が見つかりません')
      if (document.status !== DOCUMENT_STATUS.SUBMITTED) {
        throw new ConflictError('DOCUMENT_NOT_PENDING', 'この書類は審査済みです')
    ...
    hasApprovedIdentity(memberId) {
      const doc = latestOfKind(memberId, DOCUMENT_KIND.IDENTITY)
